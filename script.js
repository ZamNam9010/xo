...
function checkWin(pMoves){
    for (const move of pMoves) {
        for (const d of dWin) {
            if(!pMoves.some(m=>{return move[0]+d[0]==m[0] && move[1]+d[1]==m[1]}) ||
                !pMoves.some(m=>{return move[0]+d[0]*2==m[0] && move[1]+d[1]*2==m[1]}) ||
                !pMoves.some(m=>{return move[0]+d[0]*3==m[0] && move[1]+d[1]*3==m[1]}) ||
                !pMoves.some(m=>{return move[0]+d[0]*4==m[0] && move[1]+d[1]*4==m[1]}) ||
                pMoves.some(m=>{return move[0]+d[0]*5==m[0] && move[1]+d[1]*5==m[1]}) ||
                pMoves.some(m=>{return move[0]-d[0]==m[0] && move[1]-d[1]==m[1]})){
                continue;
            }else{
                changeWincolor(move,d);
                return true;
            }
        }
    }
